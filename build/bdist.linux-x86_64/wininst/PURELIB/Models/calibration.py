...
class CalibrationSection(Variable):
    ...
    def set(self,  centerPupil, pos, indexXMin, indexXMax):
        ...
        centerPupil = np.delete(centerPupil, np.where(abs(centerPupil-np.median(centerPupil, axis=0)) > self.variance), axis=0)
...
class Calibration(EyeTrackingClient, Variable):
    ...
    def calibrationApply(self):
        ...
        u = np.empty((len(x),3), float)
        u[:,0] = x
        u[:,1] = y
        u[:, 2] = np.ones((1,len(x)))
        print(u)
        #e = (u-self._meanCalibration)*(self._ratioCalibration/np.array([self.width, self.height]))
        mean = np.empty((0, 3), int)
        pos = np.empty((0, 3), int)
        for calibrationSection in self.sectionCalibration:
            meanTmp = np.append(self.sectionCalibration[calibrationSection].mean, np.array([1]))
            mean = np.append(mean, np.array([meanTmp]), axis=0)
            posTmp = np.append(self.sectionCalibration[calibrationSection].pos[0, :], np.array([1]))
            pos = np.append(pos, np.array([posTmp]), axis=0)
        self.A = np.dot(np.linalg.pinv(mean), pos)
        print(self.A)
        result = np.dot(u, self.A)
        result = np.delete(result, 2, 1)
        print(result)
        #test = (u - self._meanCalibration) * self._ratioCalibration
        return result/np.array([self.width, self.height])
    ...
    def calibrationApplyClient(self, centersPupil):
        ...
        centersPupilTmp = np.append(u, np.array([1]))
        result = np.dot(centersPupilTmp, self.A)
    ...
    def plot(self):
        ...
        for calibrationSection in self.sectionCalibration:
            ...
            ax1.text((xMin + xMax) / 2, max(self.centersPupil[:, 0]), calibrationSection, horizontalalignment='center',
                     fontsize=8)
            ax1.axvline(x=xMin)
            ax2.text((xMin + xMax) / 2, max(calibrationValue[:, 0]), calibrationSection, horizontalalignment='center',
                     fontsize=8)
            ax2.axvline(x=xMin)
            ax1.axvline(x=xMax)
