...
class Variable(QtCore.QObject):
    ...
    def writeData(name, envVariable):
        output_file = open(Variable.getName(name), 'wb')
        output_file.write(etree.tostring(envVariable))
        output_file.close()

    def getName(name):
        return os.path.abspath(name.replace('file://', ''))
...
class EnvVariable(Variable):
    ...
    def __init__(self, parent=None):
        ...
        self.load("Data/EnvVariable/Default.xml")
    ...
    def reset(self):
        ...
        self.setFileEyeTrackingVariable("Data/EyeTrackingVariable/Default.xml")
        self.setFileCalibration("Data/Calibration/Default.xml")
